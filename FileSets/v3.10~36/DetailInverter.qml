...
MbPage {
	...
	VBusItem
	{
		id: preferRenewableEnergyItem
		bind: Utils.path(inverterService, "/Dc/0/PreferRenewableEnergy")
	}
	VBusItem
	{
		id: remoteGeneratorSelectedItem
		bind: Utils.path(inverterService, "/Ac/State/RemoteGeneratorSelected")
	}
	property bool showChargePriority: numberOfMultis > 0 && preferRenewableEnergyItem.valid
	property bool preferRenewableEnergy: showChargePriority && preferRenewableEnergyItem.value == 1
	property bool autoReturnToRenewable: remoteGeneratorSelectedItem.valid
	property bool acInIsGenerator: remoteGeneratorSelectedItem.valid && remoteGeneratorSelectedItem.value == 1
	...
	Column
	{
		...
		Row
		{
			...
			DetailButton
			{
				...
				height: root.buttonHeight
				width: root.buttonWidth
				onClicked: { if (! acInIsGenerator) preferRenewableEnergyItem.setValue (0)}
				visible: showChargePriority
				content: TileText
				{
			...
			DetailButton
			{
				...
				width: root.buttonWidth
				visible: showChargePriority
				onClicked: { if (! acInIsGenerator) preferRenewableEnergyItem.setValue (1)}
				content: TileText
				{
					text: qsTr("Renew\nable"); font.bold: true
					color: preferRenewableEnergy && ! acInIsGenerator ? "white" : "gray"
